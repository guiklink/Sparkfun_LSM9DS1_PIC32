...
static volatile enum {IDLE, START, WRITE, READ, RESTART, SAK, ACK, NACK, STOP, ERROR} state = IDLE;   // keeps track of the current i2c state
  ...
  switch(state) {
      ...
      if(I2C1STATbits.ACKSTAT) {                // did not receive a nack from the slave, this is an error
        ...
        if(write_index < n_write) {             // still more data to write
          ...
          if(n_read > 0) {                      // we want to read so issue a restart
            ...
            state = STOP;
            I2C1CONbits.PEN = 1;                // Initiate stop sequence 
          }
        }
      }
      break;
    ...
    case READ:
      ...
      if(read_index == n_read) { // we are done reading, so send a nack
        state = NACK;
        I2C1CONbits.ACKDT = 1;    // aplicable during master receive, 1 = send NACK | 0 = send ACK
      } else {
        state = ACK;
        I2C1CONbits.ACKDT = 0;
      }
    ...
    case ACK:
      ...
      state = READ;
      I2C1CONbits.RCEN = 1;                 // Enables receive mode
      break;
    case NACK:
      //issue a stop
      state = STOP;
